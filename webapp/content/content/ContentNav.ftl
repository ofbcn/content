<#--
  ...
  under the License.
  -->
<script language="javascript" type="text/javascript" src="<@noerpContentUrl>/assets/jquery/plugins/jsTree/jquery.jstree.js</@noerpContentUrl>"></script>
      ...
      <#macro fillTree assocList>
          <#if (assocList?has_content)>
            <#list assocList as assoc>
                <#assign content  = delegator.findOne("Content",Static["org.noerp.base.util.UtilMisc"].toMap("contentId",assoc.contentIdTo), false)/>
                {
                ...
                <#assign assocChilds  = delegator.findByAnd("ContentAssoc",Static["org.noerp.base.util.UtilMisc"].toMap("contentId",assoc.contentIdTo,"contentAssocTypeId", "TREE_CHILD"), null, false)/>
...
jQuery(document).ready(createTree());
 ...
 <#-------------------------------------------------------------------------------------define Requests-->
  var editDocumentTreeUrl = '<@noerpUrl>/views/EditDocumentTree</@noerpUrl>';
  var listDocument =  '<@noerpUrl>/views/ListDocument</@noerpUrl>';
  var editDocumentUrl = '<@noerpUrl>/views/EditDocument</@noerpUrl>';
  var deleteDocumentUrl = '<@noerpUrl>removeDocumentFromTree</@noerpUrl>';
...
<#-------------------------------------------------------------------------------------callDocument function-->
    function callDocument(contentId) {
        ...
        if (tabitem=="navigateContent")
            listDocument = '<@noerpUrl>/views/ListDocument</@noerpUrl>';
        if (tabitem=="LookupContentTree")
            listDocument = '<@noerpUrl>/views/ListContentTree</@noerpUrl>';
        if (tabitem=="LookupDetailContentTree")
            listDocument = '<@noerpUrl>/views/ViewContentDetail</@noerpUrl>';
