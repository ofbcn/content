...
<#if contentAssoc?has_content>  
      ...
      <#elseif tabButtonItem=="ListDocument">
          ...
          <tr <#if alt_row> class="alternate-row"</#if>>
             ...
             <#assign caFromDate = Static["org.noerp.base.util.UtilDateTime"].toDateString(contentData.caFromDate, "dd/MM/yyyy")/>
            </#if> 
              <td>${caFromDate!}</td>
              <td><a href="javascript:document.listDocumentForm_${listcount}.submit()" >${uiLabelMap.CommonDelete}</a></td>
          </tr>
          <form action="<@noerpUrl>removeDocumentFromTree</@noerpUrl>" name="listDocumentForm_${listcount}" method="post">
